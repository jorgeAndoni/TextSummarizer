...
class Accessibility(object):
    ...
    def get_accs_values(self, h):
        ...
        values = values[:len(values) - 1]
        values = [float(i) for i in values]
        return (reverseSortList(values), values)
...
class GeneralizedAccesibility(object):
    ...
    def calculate_accesibility(self):
        ...
        for i in range(N):
            value = np.exp(-np.sum(P[i, :] * np.log(P[i, :])))
            if math.isnan(value):
                va[i] = 0
            else:
                va[i] = value
    ...
    def get_gaccs_values(self):
        measures = self.calculate_accesibility()
        measures = measures.tolist()
        return (utils.reverseSortList(measures), measures)